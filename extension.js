  ...
  class WindowsInPanelTaskbar extends St.Widget {
    ...
    _syncWindowSignals(windows) {
      ...
      for (const [window, signalIds] of this._windowSignalIds) {
        if (visibleWindows.has(window)) continue;
      ...
      for (const window of windows) {
        if (this._windowSignalIds.has(window)) continue;
    ...
    _buildEntries(windows) {
      ...
      for (const window of windows) {
        ...
        if (!windowsByAppId.has(appId)) windowsByAppId.set(appId, []);
      ...
      for (const window of windows) {
        ...
        if (app && favoriteIds.has(app.get_id())) continue;
    ...
    _createIcon(entry) {
      if (entry.app) return entry.app.create_icon_texture(ICON_SIZE);
    ...
    _createButton(entry, focusedWindow) {
      ...
      if (shortcutBadge) content.add_child(shortcutBadge);

      const runningIndicator = this._createRunningIndicator(
        entry,
        focusedWindow,
      );
      if (runningIndicator) content.add_child(runningIndicator);
      ...
      button.connect("clicked", () => {
        ...
        if (entry.type === "launcher") entry.app.open_new_window(-1);
        else entry.window.activate(global.get_current_time());
      });
...
export default class WindowsInPanelExtension extends Extension {
  ...
  _moveDateMenu(active) {
    ...
    if (active) {
      ...
      if (!panel.right.includes("dateMenu")) panel.right.unshift("dateMenu");
    } else {
